...
When presenting this project during engineering interviews, highlight these core technical implementations:

1. **Patient History Dashboard & MongoDB Aggregation:** Implemented efficient single-query analytics in `GET /api/sessions/summary` utilizing `$lookup` joins, `$filter`, `$size`, `$substrCP`, and date grouping.
2. **Dual AI Model Streaming Architecture:** Primary integration with Google Gemini Flash for low-latency streaming, with automatic fallback to HuggingFace `Qwen2.5-72B` for high availability.
3. **In-Browser Document RAG Engine:** Multi-stage client-side text extraction using `pdfjs-dist` for PDFs and Gemini Flash Vision OCR for medical images, injecting extracted context directly into AI system prompts.
4. **Zero-Cost External API Orchestration:** Concurrent parallel fetch using `Promise.allSettled()` to search WHO statistics, WHO outbreak events, and PubMed medical research papers simultaneously.
